...
## 2026-09-12: pre-submission checks complete; metadata fix; tarball rebuilt; SUBMITTED

- **Metadata fix.** `inst/CITATION` (second bibentry), `NEWS.md` (0.6.0 header) and `README.md` (Citation section) still carried the retired working title "When is a diagram a model? ..."; all three now read "The Unified Model Graph: Drawing the Statistical Models of Psychology" (NEWS.md uses the short form inline). Citation note changed to "Manuscript in preparation for submission" to match HT's sequencing (CRAN first, then BRM). DESCRIPTION and inst/WORDLIST untouched.
- **Phase 1, local check** (Windows 11 x64, R 4.5.3 ucrt): `devtools::test()` 313 PASS / 0 FAIL / 2 WARN (both are `lme4::findbars()`/`nobars()` deprecation notices pointing to the `reformulas` package; not a check failure; schedule the migration for 0.6.1/0.7.0). `R CMD check --as-cran`: 0 errors, 0 warnings, 1 NOTE ("detritus in the temp directory: lastMiKTeXException", a local MiKTeX artifact).
- **Phase 2, win-builder** R-devel (2026-09-10 r90519) and R-release (4.6.1): both 0 errors, 0 warnings, one NOTE each (New submission; "possibly misspelled" UMG and observability, both false positives explained in cran-comments.md). Logs in `winbuilder_devel/`.
- **macOS builder** (`devtools::check_mac_release()`) failed twice on infrastructure errors (FTP upload failure, then HTTP 502 from mac.r-project.org); the package built both times. HT decision: proceed without the macOS pre-check (not a CRAN requirement).
- `umg/cran-comments.md` finalized with the three real environments and the false-positive explanations. Tarball rebuilt (`umg_0.6.0.tar.gz`, 2026-09-12).
- **Submitted to CRAN (umg 0.6.0, first submission).** Confirmation e-mail link clicked per Phase 4 of the guide. [HT: add the exact upload date and time here.]

## 2026-09-15: status

- **CRAN: awaiting the incoming-review result.** `crandb.r-pkg.org/umg` returned 404 on 2026-09-15, so the package is not yet on CRAN. Typical first-submission latency is a few days to two weeks; a human reviewer's request for small DESCRIPTION/Rd changes is common; budget one resubmission as 0.6.1.
- **The BRM manuscript was submitted 2026-09-12 (BR-Org-26-1143) and cites umg 0.6.0 as "submitted to CRAN and awaiting feedback".** Do not change any package behaviour that the manuscript's numbers depend on until the BRM decision; documentation-only and metadata changes are fine.
- **On CRAN acceptance (Phase 5 of the guide):** tag `v0.6.0` on GitHub at the commit matching the accepted tarball; add the CRAN badge to README; update `inst/CITATION`/`README.md` note to "Manuscript submitted for publication" (BRM, 2026-09-12); mint a Zenodo DOI for the GitHub release; build the pkgdown site; announce (R-bloggers/Posit Community post, one figure); then prepare the R Journal submission (`_0_R_package/paper_rjournal/yu-umg.Rmd`, update to 0.6.x; submit after the BRM decision so the two papers do not overlap in review at the same time).
- **On a CRAN request for changes:** make only the requested edits, bump to 0.6.1, re-run `R CMD check --as-cran` locally and on win-builder, update cran-comments.md with the reviewer's points answered one by one, resubmit. Keep the manuscript-facing behaviour identical.
- **Still uncommitted on 2026-09-12:** the CITATION/NEWS/README/cran-comments.md/.Rbuildignore edits (git status showed `??` for `.Rbuildignore` and `cran-comments.md`). Commit and push them so the tagged source matches the submitted tarball. Re-run `git status` before trusting this line.
- Next package work after acceptance (0.7.0): `umg_diff()`; `lme4::findbars()` to `reformulas`; TikZ/PDF vignette; benchmark vignette; consider a documented JSON export of the object model (`umg.json`) as the interchange format.
